...
3. 第一次開啟若提示缺 runtime，按「一鍵安裝 runtime」。自動安裝失敗時，下載
   `RealtimeAudioTranslator-runtime-cuda12-core-<tag>.7z` 與 `RealtimeAudioTranslator-runtime-cuda12-dlls-<tag>.zip`，解壓到同一暫存資料夾，
   再於程式內按「手動匯入 runtime」選擇該資料夾；程式會驗證後安裝到 `%USERPROFILE%\.realtime-audio\runtime\cuda12`。
