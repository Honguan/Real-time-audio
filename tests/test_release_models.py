...
class ReleaseModelsTests(unittest.TestCase):
    ...
    def test_start_checks_model_before_engine(self):
        ...
        self.assertIn('self.runtime_service.check(target, config.get("device", "auto"), config.get("compute_type", "auto"), verify_hashes=True)', gui_source)
        ...
        self.assertIn('install_message = self._t("runtime 尚未就緒；請開啟 runtime 資料夾或匯入 runtime") if self.config.get("app_language") == "en" else runtime_install_message(runtime_dir(self.config), self.config.get("device", "auto"))', gui_source)
        self.assertIn('messagebox.showerror(self._t("runtime 不可用"), error + "\\n\\n" + install_message)', gui_source)
        self.assertIn('self._set_last_error(error)', gui_source)
        self.assertIn("app_models = models_dir(config)", gui_source)
        self.assertIn('self.model_service.status(config["model"], self.repo_root / "_models", app_models)', gui_source)
        self.assertIn("if current_model_status != MODEL_READY:", gui_source)
        self.assertIn('messagebox.showerror(title, self._t("請下載或重新安裝模型：{model}", model=config["model"]) if self.config.get("app_language") == "en" else model_install_message(config["model"], app_models))', gui_source)
