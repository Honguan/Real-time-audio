...
3. 第一次啟動若提示缺 runtime，到 https://github.com/Purfview/whisper-standalone-win/releases 下載 Faster-Whisper-XXL Windows runtime，並下載 `cuBLAS.and.cuDNN_CUDA12_win_v3.7z`，兩個都解壓到：

```text
%USERPROFILE%\.realtime-audio\runtime\cuda12
```

該資料夾內應直接看到 `faster-whisper-xxl.exe`。主程式 zip 已包含 Python runtime，不需要另外安裝 Python 或 venv。
...
- NVIDIA CUDA12 runtime：到 https://github.com/Purfview/whisper-standalone-win/releases 下載 Faster-Whisper-XXL Windows runtime 和 `cuBLAS.and.cuDNN_CUDA12_win_v3.7z`
...
預設 `Provider` 可使用本機模式，不會呼叫雲端翻譯。若要使用雲端：
...
- 找不到 runtime：下載 Faster-Whisper-XXL Windows runtime 和 `cuBLAS.and.cuDNN_CUDA12_win_v3.7z`，兩個都解壓到 `%USERPROFILE%\.realtime-audio\runtime\cuda12`。
