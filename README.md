...
- NVIDIA CUDA12 建議下載：`RealtimeAudioTranslator-runtime-cuda12-<tag>.zip`
...
- 找不到 runtime：下載 runtime zip，解壓到 `%USERPROFILE%\.realtime-audio\runtime\cuda12`。
...
- GPU 無法使用：把 `Device` 改成 CPU 或確認 CUDA12 runtime zip 已正確解壓。
