...
3. 第一次啟動若提示缺 runtime，到同一個 Release 下載 `RealtimeAudioTranslator-runtime-cuda12-<tag>.zip`，解壓到：
...
- NVIDIA CUDA12 runtime：`RealtimeAudioTranslator-runtime-cuda12-<tag>.zip`
...
- 找不到 runtime：下載 `RealtimeAudioTranslator-runtime-cuda12-<tag>.zip`，解壓到 `%USERPROFILE%\.realtime-audio\runtime\cuda12`。若 Release 沒有 runtime zip，可改到 https://github.com/Purfview/whisper-standalone-win/releases 下載 Faster-Whisper-XXL Windows runtime 和 `cuBLAS.and.cuDNN_CUDA12_win_v3.7z`。
